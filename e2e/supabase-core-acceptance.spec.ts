...
import { chooseCustomOption, gotoSidebar } from './helpers';
...
test.describe('Supabase Kernabnahme – authentifizierter Browserlauf', () => {
  ...
  test('Admin: Login, Session, Rollenschutz', async ({ page }) => {
    await assertProtectedRouteRedirectsToLogin(page, '/sales');
    await loginWithPassword(page, credentials.adminEmail, credentials.adminPassword);
    await expect(page.getByRole('combobox', { name: 'Demo-Benutzer wechseln' })).toHaveCount(0);

    await page.reload();
    await expect(page.getByRole('heading', { name: 'Arbeitsplatz' })).toBeVisible({ timeout: 20_000 });

    await page.goto('/admin/users');
    await expect(page.getByRole('heading', { name: 'Benutzer', level: 1 })).toBeVisible();
  });

  test('Admin: Kunde anlegen, bearbeiten, Betreuer, Suche', async ({ page }) => {
    await loginWithPassword(page, credentials.adminEmail, credentials.adminPassword);
    ...
    await page.locator('li, article, div').filter({ hasText: TEST_COMPANY }).first().getByRole('button', { name: 'Bearbeiten' }).click();
    ...
    await gotoSidebar(page, 'Kunden');
    const allLeadLinks = page.getByRole('link').filter({ has: page.locator('h3, [class*="title"]') });
  ...
  test('Admin: Beratung manuell 12,50 € mit Reload', async ({ page }) => {
    await loginWithPassword(page, credentials.adminEmail, credentials.adminPassword);
    ...
    await page.reload();
    await expect(page.getByRole('heading', { name: 'Ausgangslage' })).toBeVisible({ timeout: 20_000 });
    await expect(page.getByLabel('Monatliche Ist-Gesamtkosten (EUR)')).toHaveValue(/12,50\s*€/);
  ...
  test('Admin: Beratung 0 € mit Reload', async ({ page }) => {
    await loginWithPassword(page, credentials.adminEmail, credentials.adminPassword);
  ...
  test('Admin: Standardprovision und Mitarbeitervereinbarung mit Reload', async ({ page }) => {
    await loginWithPassword(page, credentials.adminEmail, credentials.adminPassword);
  ...
  test('Admin: Angebot, PDF, Workflow-Status mit Reload', async ({ page }) => {
  ...
  test('Außendienst: Login, RLS, Sichtbarkeit, Schreibschutz', async ({ page }) => {
    test.skip(!testLeadId, 'Kein Testkunde');
    await logout(page);
    await loginWithPassword(page, credentials.fieldEmail, credentials.fieldPassword);
    await expect(page.getByRole('combobox', { name: 'Demo-Benutzer wechseln' })).toHaveCount(0);
  ...
  test.afterAll(async () => {
    // Bereinigung erfolgt über scripts/supabase-browser-acceptance-cleanup.mjs nach dem Lauf.
  });
});
