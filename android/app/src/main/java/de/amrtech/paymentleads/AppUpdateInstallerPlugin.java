...
public class AppUpdateInstallerPlugin extends Plugin {
    ...
    public static final String MSG_INSTALL_SOURCE_BLOCKED =
            "install_source_blocked: Falls Android die Installation blockiert: unter Einstellungen die Installation aus "
                    + "dieser Quelle für AMRtech Payment erlauben („Apps aus dieser Quelle zulassen“). Danach:"
                    + " Button „Update installieren“ erneut tippen.";
    ...
    private static boolean canInstallPackagesNow(final PackageManager pm) {
    ...
    @PluginMethod
    public void canInstallPackages(final PluginCall call) {
        final PackageManager pm = getContext().getPackageManager();
        final JSObject ret = new JSObject();
        ret.put("canInstall", canInstallPackagesNow(pm));
        call.resolve(ret);
    }
    ...
    public void openInstallPermissionSettings(final PluginCall call) {
        ...
        try {
            ...
            call.reject("Android-Einstellungen für App-Installation konnten nicht geöffnet werden.", e);
        } catch (final Exception e) {
            call.reject("Einstellungen konnten nicht geöffnet werden.", e);
        }
    }

    /**
     * @param relativePath Relativ zum internen Cache z.&nbsp;B. {@code amrtech-payment-updates/amrtech-payment-update-7.apk}.
     */
    @PluginMethod
