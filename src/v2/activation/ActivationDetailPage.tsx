...
export function ActivationDetailPage() {
  ...
  if (isLoading && !activation) {
    return (
      <section>
        <PageHeader title="Aktivierung" description="Daten werden geladen…" />
  ...
  const activationGroup = getActivationDisplayGroup(activation.status);

  return (
    <section>
      <PageHeader
        title={activation.activationNumber}
        description={`Vertrag ${activation.contractId}`}
        actions={
          <div className={styles.headerActions}>
            ...
            <Link to={`/contracts/${activation.contractId}`}>
              <Button variant="secondary">Vertrag öffnen</Button>
            </Link>
            {primaryHeaderAction}
          </div>
        }
      ...
      <div className={styles.headerMeta}>
        <StatusBadge
          variant={activationStatusVariant(activationGroup)}
          label={getActivationDisplayLabel(activation.status)}
          technicalLabel={currentUser.role === 'admin' ? getActivationTechnicalLabel(activation.status) : undefined}
        />
      ...
      {hasPermission(currentUser.role, 'activations.update') ? (
        <div className={styles.actions}>
          {hasPermission(currentUser.role, 'activations.go_live') && activation.status === 'live' ? (
            <Button variant="text" onClick={() => setReasonDialog('revoke_go_live')}>
          ...
          !['completed', 'cancelled', 'archived'].includes(activation.status) ? (
            <Button variant="text" onClick={() => setReasonDialog('cancel')}>
              Abbrechen
            </Button>
          ) : null}
        </div>
      ...
      {tab === 'documents' ? (
        <div className={styles.section} role="tabpanel" id="panel-documents" aria-labelledby="tab-documents">
          ...
          {hasPermission(currentUser.role, 'activations.documents') ? (
            ...
            >
              <FormField type="select" label="Checklistenpunkt" value={documentChecklistItemId} onChange={(event) => setDocumentChecklistItemId(event.target.value)}>
              ...
              <FormField type="select" label="Dokumenttyp" value={documentType} onChange={(event) => setDocumentType(event.target.value as SalesDocumentType)}>
              ...
              <FormField type="text" label="Dateiname" required value={documentFileName} onChange={(event) => setDocumentFileName(event.target.value)} />
      ...
      {tab === 'open_items' ? (
        <div className={styles.section}>
          ...
          {hasPermission(currentUser.role, 'activations.applications') ? (
            ...
            >
              <FormField type="select" label="Typ" value={newApplicationType} onChange={(event) => setNewApplicationType(event.target.value as ActivationApplicationType)}>
              ...
              <FormField type="text" label="Titel" required value={newApplicationTitle} onChange={(event) => setNewApplicationTitle(event.target.value)} />
      ...
      {tab === 'setup_test' ? (
        <div className={styles.section} role="tabpanel" id="panel-setup_test" aria-labelledby="tab-setup_test">
          ...
          {hasPermission(currentUser.role, 'activations.setup') ? (
            ...
            >
              <FormField type="text" label="Notiz zur Einrichtung" value={setupNote} onChange={(event) => setSetupNote(event.target.value)} />
          ...
          {hasPermission(currentUser.role, 'activations.test') ? (
            ...
            >
              <FormField type="select" label="Gerät (optional)" value={testHardwareId} onChange={(event) => setTestHardwareId(event.target.value)}>
              ...
              <FormField type="text" label="Anonymisierte Referenz" required placeholder="z. B. TEST-0001 (keine Kartendaten)" value={testAnonymizedReference} onChange={(event) => setTestAnonymizedReference(event.target.value)} />
              <div className={styles.inlineActions}>
      ...
      {tab === 'open_items' ? (
        <div className={styles.section}>
          ...
          {hasPermission(currentUser.role, 'activations.blockers') ? (
            ...
            >
              <FormField type="select" label="Kategorie" value={newBlockerCategory} onChange={(event) => setNewBlockerCategory(event.target.value as ActivationBlockerCategory)}>
              ...
              <FormField type="select" label="Schweregrad" value={newBlockerSeverity} onChange={(event) => setNewBlockerSeverity(event.target.value as ActivationBlockerSeverity)}>
              ...
              <FormField type="text" label="Titel" required value={newBlockerTitle} onChange={(event) => setNewBlockerTitle(event.target.value)} />
              <FormField type="text" label="Beschreibung" required value={newBlockerDescription} onChange={(event) => setNewBlockerDescription(event.target.value)} />
      ...
      >
        <p>Bitte bestätigen Sie diese kritische Aktivierungsaktion.</p>
      </Dialog>

      <Dialog
        isOpen={reasonDialog !== null}
        title={reasonDialog === 'cancel' ? 'Aktivierung abbrechen' : 'Go-live zurücknehmen'}
        onClose={() => {
